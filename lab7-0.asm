...
Initial:				; ????????
    ...
    CLRF LATA			; ????
    BSF TRISB,  0		; ?portB?bit0????
    BCF RCON, IPEN		; ?priority bit??
    // BCF INTCON, INT0IF		; ??Interrupt flag bit??
    BSF INTCON, GIE		; ?Global interrupt enable bit?????????ISR
    ...
    ; the register for counting : TMR1L TMR1H
    ; PTR1 : interrupt flag bit
    ; PIE1 : Overflow interrupt enable bit
    ; PTR1 : overflow interrupt priority bit
